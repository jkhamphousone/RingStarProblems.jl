function ilp_st_optimize_explore!(m, x, y, x′, y′, f, F, B, inst, pars, start_time, gurobi_env, subtourlazy_cons, nsubtour_cons, ncall_bbc; log_level=3)
    ...
    return (m, ilp_time, blossom_time, TL_reached, relative_gap(m), objective_value(m), objective_bound(m), value.(x), value.(y), nsubtour_cons, nconnectivity_cuts, nedges_cuts, nblossom, MOI.get(m, MOI.NodeCount()))
end
