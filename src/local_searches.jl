function run_two_opt_wiki(x_opt, x′, hubs, previous_cost, pars, n, r, rp, tildeV)
     ...
     while improving
        ...
        for i in 1:h
            for counter in 1:h-3
                ...
                saving = r[mima(R[i], R[i_next])] + r[mima(R[j], R[j_next])] - (r[mima(R[i], R[j])] + r[mima(R[i_next], R[j_next])])
                if R[i] in tildeV
                ...
                if saving > 0 # Move i-j accepted
                    ...
                    improved_x′[mima(R[i_nenext], R[j_next])...] = true*(R[i_next] in tildeV)
                    improved_x′[mima(R[i], R[j_prev])...] = true*(R[j] in tildeV)
                    improved_x′[mima(R[i_next], R[j_nenext])...] = true*(R[j_next] in tildeV)
    ...
    pars.two_opt == 1 && @info "Finished 2-opt LocalSearch at $improved_cost <= $previous_cost with $(number_moves) moves"
