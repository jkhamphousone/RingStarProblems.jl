module PlottingExt
    ...
    function RingStarProblems.perform_plot(
        ...
        nodelabels =
            String[i == result_table.sol.j★ ? "$i\n $(round(result_table.sol.B,digits=2))" : "$i" for i = 1:n]
        ...
        pdfpath = eval(@__DIR__) *"/results/$alpha_or_nrand/$(alpha_or_nrand)_TL-$(pars.timelimit)$(two_opt_string)_F=$(inst.F)_UB=$(round(result_table.UB))).pdf"
        draw(
            PDF(pdfpath),
            gplot(
                inst_graph,
