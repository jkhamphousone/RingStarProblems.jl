...
\begin{figure}[ht!]
    ...
    \draw[axe](0,0) -- (0.0,0);
     \draw[axe](0,0) -- (0,11.51700000000000177);\draw (\F[0],0) -- (\F[0],-.2);
...
\node (F) at (2.0,2){}; \draw(F) node{$F$};
    \node (g) at (2,11.517000000000001){}; \draw(g) node{$g(F)$};
    % Lines
...
\begin{itemize}
	\item  $F = [0.0,0.0]$
 \item $B = [2.34,0.42]$
 \item  $K = [1047.0,1047.0]$\item Total execution time is: 12.13s\item Number of times BBC is called: 2\end{itemize}\end{document}