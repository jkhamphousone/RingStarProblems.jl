...
\begin{figure}[ht!]
    ...
    \draw[axe](0,0) -- (0.0,0);
     \draw[axe](0,0) -- (0,14.05877);\draw (\F[0],0) -- (\F[0],-.2);
...
\node (F) at (2.0,2){}; \draw(F) node{$F$};
    \node (g) at (2,14.058){}; \draw(g) node{$g(F)$};
    % Lines
...
\begin{itemize}
	\item  $F = [0.0,0.0]$
 \item $B = [0.81,0.18]$
 \item  $K = [1278.0,1278.0]$\item Total execution time is: 20.44s\item Number of times BBC is called: 2\end{itemize}\end{document}