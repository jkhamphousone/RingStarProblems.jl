...
\begin{figure}[ht!]
    ...
    \draw[axe](0,0) -- (0.0,0);
     \draw[axe](0,0) -- (0,248.88600000000002);\draw (\F[0],0) -- (\F[0],-.2);
...
\node (F) at (2.0,2){}; \draw(F) node{$F$};
    \node (g) at (2,248.88600000000002){}; \draw(g) node{$g(F)$};
    % Lines
...
\begin{itemize}
	\item  $F = [0.0]$
 \item $B = [21.18]$
 \item  $K = [22626.0]$\item Total execution time is: 49.16s\item Number of times ILP is called: 2\end{itemize}\end{document}