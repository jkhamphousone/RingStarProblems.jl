...
%     \draw[axe](0,0) -- (0.0,0);
%      \draw[axe](0,0) -- (0,474.50700000000006);\draw (\F[0],0) -- (\F[0],-.2);
...
%     \node (g) at (2,474.50700000000006){}; \draw(g) node{$g(F)$};
%     % Lines
...
%     \end{tikzpicture}
    \end{center}
    \vspace*{-2eM}
    \caption{$g(F)$ for the lin105 instance}\label{fig:1}
    \end{figure}
...
\begin{itemize}
	\item  $F = [0.0]$
 \item $B = [21.72]$
 \item  $K = [43137.0]$\item Total execution time is: 116.28s\item Number of times BBC is called: 2\end{itemize}\end{document}