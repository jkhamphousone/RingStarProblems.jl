...
function main(str)
    ...
    if !ilp_out_of_memory && !bbc_out_of_memory
        ...
        total_time_ILP_string =
            total_shift > 0 ? "\\mtcN{TL}" : "\\mtcN{$(parse(Float64,data_str[38]))}"
        ...
        total_time_Ben_string =
            total_shift_ben > 0 ? "\\mtcN{TL}" :
            "\\mtcN{$(parse(Float64,data_str[id_Ben+8]))}"
...
function html_to_tex_both_TL_all(writef = true)
    ...
    for dir in readdir("./html/solutions/journal_article_2023")
        if !(dir in ["random_instance", "clearhtml.jl"]) && dir[1:3] != "old" # REMOVE kroA150 and 200
    ...
    for dir in instances_arr
        dir_name =
            sort(readdir("./html/solutions/journal_article_2023/$(dir)/"), lt = natural)
        for d in dir_name
            str = ""
            @show d
            str = main(
                chop(
                    read("./html/solutions/journal_article_2023/$(dir)/" * d, String),
                    head = 44,
                ),
            )
            @show dir
