...
function html_to_tex_both_TL(str) ## OLD
    ...
    if data_str[16 + separate_shift] != "ILP"
        ...
        while data_str[53+date_shift+solution_printing_shift] != "BD" && solution_printing_shift <= 10000
...
function html_to_tex_both_TL_all(writef = true)
    ...
    for dir in instances_arr
        str = ""
        @show dir
        str = html_to_tex_both_TL(chop(read("./html/solutions/journal_article_2023/$(dir)/"*readdir("./html/solutions/journal_article_2023/$(dir)")[end], String), head=44))
        @show str
        @show dir
        if writef
            write(file, str)
